...
function playerCell(player: LolPlayerState | null, side: Side): string {
  ...
  return `<div class="v2-player ${side}"><span class="v2-champion" aria-hidden="true">${escapeHtml(initials(champion))}</span><span class="v2-player-copy"><strong>${escapeHtml(name)}</strong><small>${escapeHtml(champion)}</small></span><span class="v2-player-stat kda"><small>KDA</small><strong>${kda(player)}</strong></span><span class="v2-player-stat cs"><small>CS</small><strong>${formatNumber(player?.creepScore ?? null)}</strong></span><span class="v2-player-stat gold"><small>GOLD</small><strong>${formatCompact(player?.totalGold ?? null)}</strong></span></div>`;
}
